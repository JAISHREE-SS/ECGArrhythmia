...
DEFAULT_DATA_PATH = os.path.join(SCRIPT_DIR, "data", "mitdb")
...
for idx, record_name in enumerate(records):
    ...
    for lead_idx, lead_name in enumerate(lead_names):
        ...
        canonical_lead_name = LEAD_NAME_ALIASES.get(str(lead_name).upper(), str(lead_name))
        ...
        all_lead_names.append(np.full(len(filtered_labels), canonical_lead_name, dtype=object))

        print(
            f"Lead {lead_name} -> {canonical_lead_name}: windows={len(filtered_windows)} "
